...
def select_audio_output_device():
    ...
    for i in range(p.get_device_count()):
        ...
        if dev['maxOutputChannels'] > 0:
            print(f"{i}: {dev['name']} - Каналы: {dev['maxOutputChannels']}, Частота: {int(dev['defaultSampleRate'])}Hz")
...
def audio_receiving(udp_port, chosen_device_index, channels, rate, p, stop_event):
...
def start_client(server_address, udp_port, stop_event):
    ...
    audio_receiving(udp_port, chosen_device_index, channels, rate, p, stop_event)
