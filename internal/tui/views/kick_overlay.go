...
func RenderKickOverlay(p KickOverlayParams) string {
	title := styles.AppTitle.Render(fmt.Sprintf("Kick %s", p.ClientNickname))

	var lines []string
	for i, reason := range p.Reasons {
		// Separator before recent section
		if p.RecentStartIndex > 0 && i == p.RecentStartIndex {
			lines = append(lines, styles.Help.Render("  ── recent ──"))
		}
		...
		if p.CustomEditing && i == p.CustomStartIndex {
			...
			lines = append(lines, prefix+styles.StatValueNeutral.Render("Custom: ")+inputText)
		} else {
	...
	cancelBtn := "  [Cancel]  "
	if p.FocusButton == 1 {
		kickBtn = styles.SelectedItem.Render(styles.CursorGlyph + " [Kick]  ")
	}
	if p.FocusButton == 2 {
	...
	help := styles.Help.Render("↑↓: select   Enter: confirm   Tab: focus   Esc: cancel")
