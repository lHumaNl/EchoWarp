...
// MaxClients change from the default (1) is not reported as a change because
// the first mDNS probe doesn't carry MaxClients so the client defaults to 1.
...
// change from an explicit non-default old value is reported as non-critical.
func TestCompareServerParams_MaxClients_NonCritical(t *testing.T) {
	cfg := baseConfig()
	cfg.MaxClients = 5 // explicit non-default
	probe := baseProbe()
	probe.MaxClients = 10
	changes := CompareServerParams(cfg, probe)
	require.Len(t, changes, 1)
